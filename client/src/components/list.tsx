...
export const List = (props: { data: any }) => {
  ...
  const addListItem = () => {
    setItemList([
      ...itemList,
      { name: "test", required: false, available: 1, cost: 10, assignees: [] },
    ]);
    counter += 1;
  ...
  return (
    <>
      <Table bgcolor="gray">
        ...
        <div>{JSON.stringify(yarray.toArray())}</div>
        <div>
          <button
            className="bg-gray-900 hover:bg-gray-800 text-white text-lg font-bold py-2 px-4 rounded"
            onClick={addListItem}
          >
            add new item
          </button>
